<template>
  ...
  >
    ...
    <div class="p-7" :class="popular ? 'pt-12' : ''">
      ...
      <p class="text-slate-500 dark:text-slate-400 text-xs mb-6 leading-relaxed min-h-[2.75rem]">{{ plan.description }}</p>

      <!-- Price -->
      <div class="mb-6">
        <div class="flex items-baseline gap-1">
          <span class="text-sm text-slate-500">$</span>
          <span class="text-5xl font-extrabold" :class="popular ? 'gradient-text' : 'text-slate-900 dark:text-slate-100'">
            {{ formatPriceWhole(plan.price) }}
          </span>
          <span class="text-base font-semibold text-slate-600 dark:text-slate-300">.{{ formatPriceCents(plan.price) }}</span>
      ...
      <ul class="space-y-2.5 mb-7 min-h-[200px]">
        ...
        >
          <div class="w-4 h-4 rounded-full bg-emerald-500/15 border border-emerald-500/25 flex items-center justify-center flex-shrink-0 mt-0.5">
            <svg class="w-2.5 h-2.5 text-emerald-500 dark:text-emerald-400" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">
