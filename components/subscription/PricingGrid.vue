<template>
  <div>
    ...
    <div class="flex items-center justify-center mb-12">
      <div
          class="relative inline-flex items-center gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-white/[0.04] border border-slate-300 dark:border-white/[0.08] backdrop-blur-md box-border">
        <button
            :class="!isYearly
        ...
        >
          {{ t('subs.yearly') }}
          <span
              class="px-1.5 py-px rounded-full text-[9px] font-extrabold tracking-wider text-white bg-gradient-to-br from-emerald-500 to-emerald-600">
            {{ t('subs.save') }}
          </span>
        </button>
      </div>
    ...
    <div class="mt-12 flex flex-wrap items-center justify-center gap-6 text-xs text-slate-500">
      ...
      <div class="flex items-center gap-2">
        <svg class="w-4 h-4 text-emerald-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path d="M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941" stroke-linecap="round"
                stroke-linejoin="round"/>
        </svg>
        {{ t('subs.scale') }}
      </div>
