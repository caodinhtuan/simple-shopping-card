<template>
  <div class="cart-summary glass-card p-6 lg:sticky lg:top-24">
    ...
    <p class="text-xs text-slate-500 mb-6">{{ itemCount }} {{ itemCount > 1 ? t('cart.items_count', { n: itemCount }).split(' ').slice(1).join(' ') : t('cart.item_count_single').split(' ').slice(1).join(' ') }}</p>
    ...
    <div class="mt-6 space-y-2.5">
      ...
      >
        <div class="flex items-center justify-center gap-2">
          ...
          <span>{{ t('summary.pay_stripe') }}</span>
        </div>
      </n-button>
      ...
      >
        <div class="flex items-center justify-center gap-2">
          ...
          <span>{{ t('summary.pay_paypal') }}</span>
        </div>
      </n-button>
