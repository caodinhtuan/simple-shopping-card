    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
      ...
      <div class="animate-in stagger-2">
        <div class="glass-card p-6">
          ...
          <TransitionGroup class="space-y-3 mb-6" name="review" tag="div">
            <div
      ...
      <div class="animate-in stagger-3">
        <div class="glass-card p-6">
          ...
          <div class="space-y-3">
            ...
            >
              <div :style="{ background: 'linear-gradient(135deg,#0070ba,#003087)' }"
                   class="w-10 h-10 rounded-xl flex items-center justify-center">
                <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path
...
function getEmoji(c: string) {
  return emojiMap[c] || '📦'
}

function getGradient(c: string) {
  return gradientMap[c] || 'linear-gradient(135deg,#8b5cf6,#06b6d4)'
}
